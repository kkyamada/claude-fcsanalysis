...
def main(args):
    ...
    # Path settings
    output_dir = args.experiment_dir / Path("output_fluor50")
    if not output_dir.exists():
        os.makedirs(output_dir)

    # Set up file handler for logging
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = output_dir / Path(f"quant_fluor50_{timestamp}.log")
    ...
    all_results = []
    search_dir = args.experiment_dir / Path("data")
    for data_dir in sorted(os.listdir(search_dir)):
        ...
        input_gml = args.experiment_dir / Path("output") / Path(data_dir) / Path("gating_strategy.gml")
    ...
    if all_results:
        ...
        output_csv = output_dir / f"fluor50_summary.csv"
