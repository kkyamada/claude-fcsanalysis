"""
Histogram plotting script for MOLM13 Marker Disruption experiment.
...
def extract_threshold_from_gml(gml_path: Path, gate_name: str) -> float:
    ...
    if dim is not None:
        # For APC- gate, threshold is the max value (below threshold = negative)
        max_val = dim.get(f"{{{ns['gating']}}}max")
        if max_val:
...
def main():
    ...
    rep1_dir = experiment_dir / 'output' / '260103_MOLM13_MarkerDisruption_day7_rep1'
    data_dir = experiment_dir / 'data' / '260103_MOLM13_MarkerDisruption_day7_rep1'
    ...
    threshold = extract_threshold_from_gml(gml_path, 'APC_neg')
    ...
    # Flatter panels (following EL4 GFPTransfection style)
    ...
    for sample_id in sample_ids:
        ...
        # Get Live gate membership (parent of APC-)
        live_result = gated_sample.get_gate_membership('Live')

        # Get raw events and apply logicle transform manually
        raw_events = sample.get_events(source='raw')
        apc_raw = raw_events[live_result, apc_idx]

        # Apply logicle transform (matching GML: T=16777215, W=0.5, M=4.5, A=0.5 for ZE5)
    ...
    for i, sample_id in enumerate(sample_ids):
        ...
        parts = name.split('_')
        # Format: MOLM13_Treatment_Type_Concentration or MOLM13_Mock_Concentration
        if 'Mock' in name:
            label = 'Mock'
            color = mock_color
        else:
            treatment = parts[1] if len(parts) > 1 else 'Unknown'
            sample_type = parts[2] if len(parts) > 2 else 'Unknown'
            label = f'{treatment} {sample_type}'
            color = default_color
